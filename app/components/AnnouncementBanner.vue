<template>
  <!-- Thêm absolute, top-16, left-0, right-0, z-[45] và pointer-events-none -->
  <div v-if="notice" class="absolute top-16 left-0 right-0 z-[45] mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mt-4 pointer-events-none">
    
    <Transition name="fade" mode="out-in">
      
      <!-- TRẠNG THÁI 1: KHI BỊ ẨN (Thu gọn) -->
      <!-- Thêm pointer-events-auto để người dùng bấm được vào nút này -->
      <div v-if="isDismissed" class="flex justify-end pointer-events-auto">
        <button 
          @click="toggleNotice" 
          class="flex items-center gap-2 text-xs font-medium text-zinc-400 hover:text-white transition-colors bg-white/5 hover:bg-white/10 px-4 py-2 rounded-full border border-white/10 shadow-sm backdrop-blur-md"
        >
      ...
      <!-- Thêm pointer-events-auto ở đây -->
      <div v-else class="relative overflow-hidden rounded-xl bg-[#0a0a0f]/90 backdrop-blur-md border border-white/10 shadow-2xl p-4 sm:p-5 flex justify-between items-start sm:items-center group pointer-events-auto">
