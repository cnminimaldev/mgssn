<template>
  ...
  >
    ...
    >
      ...
      <!-- Header / Tiêu đề -->
      <div 
        class="relative z-10 flex items-start justify-between px-6 pt-6"
      ...
      >
        ...
        <!-- Các nút điều khiển (Đã gỡ pointer-events-auto cứng) -->
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-1 sm:gap-3">
            <button class="text-white hover:text-emerald-400 transition-colors p-1" @click.stop="togglePlay">
              <svg v-if="!isPlaying" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-8 w-8"><path fill-rule="evenodd" d="M4.5 5.653c0-1.426 1.529-2.33 2.779-1.643l11.54 6.348c1.295.712 1.295 2.573 0 3.285L7.28 19.991c-1.25.687-2.779-.217-2.779-1.643V5.653z" clip-rule="evenodd" /></svg>
              <svg v-else xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-8 w-8"><path fill-rule="evenodd" d="M6.75 5.25a.75.75 0 01.75-.75H9a.75.75 0 01.75.75v13.5a.75.75 0 01-.75.75H7.5a.75.75 0 01-.75-.75V5.25zm7.5 0A.75.75 0 0115 4.5h1.5a.75.75 0 01.75.75v13.5a.75.75 0 01-.75.75H15a.75.75 0 01-.75-.75V5.25z" clip-rule="evenodd" /></svg>
            </button>
          ...
          <div class="flex items-center gap-1 sm:gap-2">
            <!-- Quality Settings -->
            <div class="relative">
              <button class="text-white hover:text-emerald-400 transition-colors p-2 rounded-full hover:bg-white/10" :class="{ 'text-emerald-400': showSettings }" @click.stop="toggleSettings" title="画質設定">
            ...
            <div class="relative" v-if="subtitles && subtitles.length > 0">
              <button class="text-white hover:text-emerald-400 transition-colors p-2 rounded-full hover:bg-white/10" :class="{ 'text-emerald-400': showSubsMenu || activeTrackIndex !== -1 }" @click.stop="toggleSubsMenu" title="字幕">
              ...
              <div v-if="showSubsMenu" class="absolute bottom-full right-0 mb-3 w-64 overflow-hidden rounded-xl bg-zinc-900/95 p-1 shadow-2xl ring-1 ring-white/10 backdrop-blur-md">
                <!-- Giữ nguyên Menu phụ đề của bạn -->
                  ...
                  <div v-if="activeTrackIndex !== -1 && !isIOS" class="space-y-3 pt-3 border-t border-white/10">
                    <div class="space-y-1">
                      ...
                      <div class="flex flex-col gap-0.5">
                        <button v-for="font in fontOptions" :key="font.val" class="flex items-center justify-between w-full rounded px-2 py-1.5 text-xs hover:bg-white/10" :class="subSettings.fontFamily === font.val ? 'text-emerald-400 font-bold' : 'text-zinc-300'" @click="subSettings.fontFamily = font.val">
                          <span>{{ font.label }}</span><span v-if="subSettings.fontFamily === font.val">✓</span>
                        </button>
                      </div>
                    </div>
                    <div class="space-y-1">
                    ...
                    <div class="space-y-1">
                      ...
                      <div class="flex gap-1">
                        <button v-for="bg in bgOptions" :key="bg.val" class="flex-1 py-1 text-[10px] rounded border transition-colors" :class="subSettings.bgOpacity === bg.val ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-zinc-700 bg-zinc-800 text-zinc-400 hover:bg-zinc-700'" @click="subSettings.bgOpacity = bg.val">
                          {{ bg.label }}
                        </button>
                      </div>
                    </div>
                    <div class="space-y-1">
                      ...
                      <div class="flex gap-1">
                        <button v-for="edge in edgeOptions" :key="edge.val" class="flex-1 py-1 text-[10px] rounded border transition-colors" :class="subSettings.edgeStyle === edge.val ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-zinc-700 bg-zinc-800 text-zinc-400 hover:bg-zinc-700'" @click="subSettings.edgeStyle = edge.val">
                          {{ edge.label }}
                        </button>
                      </div>
                    </div>
                  </div>
                  <div v-if="isIOS && activeTrackIndex !== -1" class="pt-2 border-t border-white/10 text-[10px] text-zinc-400 text-center">※iOSでは端末の設定が適用されます</div>
                </div>
              </div>
            </div>
