    ...
    <div class="mx-auto max-w-5xl">
      <div class="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 class="text-2xl font-bold text-white">シリーズ一括登録</h1>
...
const startImport = async () => {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    try {
      ...
      // Bổ sung bẫy lỗi và tối ưu cho Series Genres
      ...
      if (genresRaw) {
        ...
        if (matchedIds.length > 0) {
          ...
          const { error: genreError } = await supabase.from('series_genres').insert(genreInserts)
          if (genreError) {
