  ...
  <div class="min-h-screen bg-[#05060a] text-zinc-300 p-6 sm:p-10">
    <div class="mx-auto max-w-5xl">
      <div class="flex items-center justify-between mb-8">
        <div>
          <NuxtLink to="/admin/series" class="text-xs text-emerald-400 hover:underline mb-2 flex items-center gap-1">
            ...
            シリーズ一覧に戻る (Quay lại danh sách Series)
          </NuxtLink>
          <h1 class="text-2xl font-bold text-white">シリーズ一括登録 (Bulk Import Series)</h1>
        ...
        <div class="space-y-6">
          <div class="bg-zinc-900/50 border border-white/5 rounded-xl p-6">
            <div class="flex items-center justify-between mb-4">
              <h2 class="text-sm font-bold text-white uppercase tracking-wider">データ入力 (Nhập dữ liệu)</h2>
            ...
            >
              ...
              {{ isProcessing ? '処理中 (Đang xử lý)...' : 'インポート開始 (Bắt đầu Import)' }}
            </button>
          </div>
        </div>

        <!-- CỘT PHẢI: TIẾN TRÌNH & LOGS -->
        <div class="space-y-6">
          <div class="bg-zinc-900/50 border border-white/5 rounded-xl p-6 h-full flex flex-col">
            ...
            <div class="mb-6">
              ...
              <!-- Danh sách các phim dính Warning -->
...
// Tái sử dụng logic Map Crew nhưng đổi type thành 'series'
const syncCrewBulk = async (contentId: number, crewString: string, role: string) => {
  ...
  for (const name of names) {
    ...
    if (pData) {
      toLink.push({ content_id: contentId, person_id: pData.id, type: 'series', role: role })
    }
  }
  return toLink
}

// BẮT ĐẦU VÒNG LẶP IMPORT
const startImport = async () => {
  if (isProcessing.value) return
  
  // 1. TIỀN XỬ LÝ (Sanitize) - Dọn sạch tàn dư của AI
  let sanitizedText = rawText.value
    .replace(/```[a-zA-Z]*\n?/g, '') 
    .replace(/```\n?/g, '')          
    .trim()

  // 2. Tách text thành các khối dữ liệu dựa theo thẻ [title]
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    const title = extractField(block, 'title') || `Untitled-Series-${Date.now()}`
    const slug = extractField(block, 'slug') || `series-${Date.now()}-${i}`
    addLog(`処理中: ${title}`, 'info')

    try {
      // 1. Chuẩn bị dữ liệu Series
      ...
      const insertData = {
        is_public: false, // Ép buộc riêng tư
        title: title,
        ...
        release_date: releaseDate, // Hoặc first_air_date tùy schema DB của bạn
        created_at: new Date(),
        updated_at: new Date(),
      }
      ...
      if (insertError) {
        // Đổi key check thành series_slug_key
        if (insertError.code === '23505' || insertError.message.includes('series_slug_key')) {
      ...
      // 3. Chèn Thể loại (Genres) vào bảng series_genres
      ...
      if (genresRaw) {
        ...
        if (matchedIds.length > 0) {
          const genreInserts = matchedIds.map(gid => ({ series_id: seriesId, genre_id: gid }))
          await supabase.from('series_genres').insert(genreInserts)
        }
      }

      // 4. Chèn Đạo diễn & Diễn viên (Crew)
      const directorLinks = await syncCrewBulk(seriesId, extractField(block, 'director'), 'director')
      const castLinks = await syncCrewBulk(seriesId, extractField(block, 'main_cast'), 'cast')
      ...
      successCount.value++
      addLog(`成功: ${title} (ID: ${seriesId})`, 'success')
  ...
  addLog('一括登録が完了しました！ (Quá trình Import đã hoàn tất!)', 'info')
}
</script>