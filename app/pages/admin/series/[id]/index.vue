      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-medium text-zinc-400 mb-1">開始年</label>
...
onMounted(async () => {
  try {
    ...
    const { data: seriesData, error } = await supabase.from("series").select("*").eq("id", seriesId).single();
    ...
    Object.assign(form, {
      ...
      origin_country: seriesData.origin_country || "JP",
...
const handleUpdate = async () => {
  ...
  try {
    ...
    if (oldData && oldData.slug !== form.slug) {
      const { error: historyError } = await supabase.from("series_slug_history").insert({ series_id: seriesId, slug: oldData.slug });
    ...
    if (genre_ids && genre_ids.length > 0) {
      ...
      const { error: gError } = await supabase.from("series_genres").insert(inserts);
