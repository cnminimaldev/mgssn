...
const loadData = async () => {
  ...
  try {
    ...
    if (sError) {
      loadError.value = sError.message
      return
    }
    ...
    if (eError) {
      loadError.value = eError.message
      return
    }

    const ep = eData as EpisodeRowFull

    if (ep.series_id !== seriesId.value || ep.collection_id !== collectionId.value) {
      loadError.value = 'シリーズIDまたはコレクションIDが一致しません。'
      return
    }
