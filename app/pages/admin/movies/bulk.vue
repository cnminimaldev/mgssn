      ...
      <div class="flex items-center justify-between mb-8">
        <div>
          <NuxtLink to="/admin/movies" class="text-xs text-emerald-400 hover:underline mb-2 flex items-center gap-1">
            ...
            映画一覧に戻る (Quay lại danh sách)
          </NuxtLink>
          <h1 class="text-2xl font-bold text-white">映画一括登録 (Bulk Import Movies)</h1>
...
const handleFileUpload = (event: Event) => {
  ...
  // [THÊM MỚI] Bổ sung dòng kiểm tra này để chiều lòng TypeScript
  if (!file) return 
...
const extractField = (text: string, tag: string) => {
  ...
  // [ĐÃ SỬA] Dùng optional chaining và kiểm tra chắc chắn match[1] tồn tại
  return (match && match[1]) ? match[1].trim() : ''
}

// Tái sử dụng logic Map Crew từ new.vue
...
const startImport = async () => {
  ...
  // [THÊM MỚI] 1. TIỀN XỬ LÝ (Sanitize) - Dọn sạch tàn dư của AI
  let sanitizedText = rawText.value
    .replace(/```[a-zA-Z]*\n?/g, '') // Xóa các thẻ như ```text, ```html
    .replace(/```\n?/g, '')          // Xóa các thẻ ``` lẻ loi
    .trim()
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    // [THÊM MỚI] Chặn đứng trường hợp block là undefined để chiều lòng TypeScript
    if (!block) continue
    
    // Đọc thông tin, nếu thiếu thì gán giá trị mặc định
    ...
    try {
      ...
      const insertData = {
        is_public: false, // Ép buộc luôn ở trạng thái Private (Nháp)
        title: title,
