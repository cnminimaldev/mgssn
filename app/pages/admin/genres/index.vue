    ...
    <div class="mx-auto max-w-4xl">
      <div class="flex items-center justify-between mb-8">
        <div>
          ...
          >
          <h1 class="text-2xl font-bold text-white">ジャンル管理 (Genres)</h1>
        </div>
        <button
          @click="openCreateModal"
          class="flex items-center gap-2 rounded-lg bg-pink-600 px-4 py-2 text-sm font-bold text-white hover:bg-pink-500 transition shadow-lg shadow-pink-900/20"
        >
          <span>＋</span> 新規作成
        </button>
      </div>
      ...
      >
        <table class="w-full text-left text-sm">
          <thead class="bg-white/5 text-xs uppercase text-zinc-400">
            <tr>
              <th class="px-6 py-4 font-medium w-32">Slug</th>
          ...
          <tbody class="divide-y divide-white/5">
            ...
            >
              <td class="px-6 py-3 font-mono text-xs text-pink-400">
                {{ g.slug }}
              </td>
              <td class="px-6 py-3 text-white">{{ g.name }}</td>
              <td class="px-6 py-3 text-zinc-300">{{ g.name_ja }}</td>
      ...
      <BaseModal
        v-model="showModal"
        :title="isEditing ? 'ジャンル編集 (Edit)' : '新規ジャンル (New)'"
      >
        ...
        <template #footer>
          ...
          <button
            @click="handleSave"
            :disabled="saving"
            class="bg-pink-600 hover:bg-pink-500 text-white px-4 py-2 rounded-lg font-bold text-sm transition flex items-center gap-2"
          >
            <span
              v-if="saving"
...
const {
  ...
  pending,
  refresh,
} = await useAsyncData("admin-genres", async () => {
...
const openCreateModal = () => {
  ...
  Object.assign(form, { slug: "", name: "", name_ja: "" });
...
const handleSave = async () => {
  if (!form.slug || !form.name || !form.name_ja) return;
  saving.value = true;
  try {
    if (isEditing.value && editingId.value) {
      ...
      await supabase.from("genres").insert({ ...form });
    }
    await refresh();
...
const deleteGenre = async (g: any) => {
  if (!confirm(`Delete "${g.name}"?`)) return;
  await supabase.from("genres").delete().eq("id", g.id);
  refresh();
};
</script>
