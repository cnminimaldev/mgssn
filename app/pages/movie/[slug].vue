        ...
        <div class="relative h-[260px] w-full overflow-hidden sm:h-[360px]">
          ...
          >
            <div class="flex items-end gap-4 sm:gap-6">
              <div
                class="hidden w-[140px] shrink-0 aspect-[2/3] overflow-hidden rounded-md border border-white/10 bg-zinc-900/80 shadow-lg sm:block"
              >
                <img
                  :src="posterUrl"
                  :alt="movie?.title"
...
type DbMovie = {
  ...
  duration_minutes?: number | null; // Cần thiết cho VideoObject schema
  created_at?: string; // Cần thiết cho VideoObject schema
  movie_genres?: {
    genre: {
      slug: string;
      name: string | null;
...
// [SEO UPDATE] Thêm Breadcrumb và VideoObject
useHead({
  link: [
    {
      rel: "canonical",
  ...
  script: [
    {
      ...
      innerHTML: computed(() => {
        ...
        const schemaVideo = {
          ...
          embedUrl: canonicalUrl.value, // Dùng tạm link hiện tại
          interactionStatistic: {
            "@type": "InteractionCounter",
            interactionType: { "@type": "WatchAction" },
            userInteractionCount: movie.value?.ratingInfo?.total_votes || 0, // Tạm dùng số vote nếu chưa có view
          },
        };
