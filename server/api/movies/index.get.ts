...
export default defineEventHandler(async (event) => {
  ...
  if (search) {
    ...
    dbQuery = dbQuery.or(
      // Thêm slug.ilike.%${search}% vào chuỗi OR
      `title.ilike.%${search}%,title_kana.ilike.%${kana}%,original_title.ilike.%${search}%,director.ilike.%${search}%,main_cast.ilike.%${search}%,slug.ilike.%${search}%`
    )
  }
