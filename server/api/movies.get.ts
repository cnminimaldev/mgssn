...
function mapDbMovieToApi(row: any) {
  ...
  return {
    ...
    // 👇 chỉ để hiển thị nhanh (label thể loại chính)
    genre: primaryGenre ?? 'その他',
    // 👇 danh sách genre chi tiết
...
export default defineEventHandler(async (event) => {
  ...
  // --- Nếu filter theo genre, cần tìm trước movie_id ---
  let genreMovieIds: number[] | null = null

  if (genreSlugs.length > 0) {
    // 1) tìm genre id tương ứng với slug
    ...
    if (genreIds.length === 0) {
      // không có genre phù hợp → chắc chắn không có phim nào
      return {
        items: [],
        total: 0,
        page,
  ...
  // filter theo movie_id thuộc các genre đã chọn
  if (genreMovieIds && genreMovieIds.length > 0) {
    builder = builder.in('id', genreMovieIds)
  }

  // search: title / kana / original_title
  if (search) {
